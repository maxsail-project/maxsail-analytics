...
- **Mejora:** La resolución de la rosa de COG ahora es configurable desde la interfaz.
  - El usuario puede definir los grados por sector (COG_BIN_SIZE) para ajustar el nivel de detalle del análisis.
- **Soporte para CSV de Vakaros:**  
  maxSail-analytics ahora acepta archivos CSV generados por dispositivos **Vakaros**, normalizando automáticamente su esquema al formato interno de la aplicación.
- **Mapeo automático de columnas:**  
  Los campos `timestamp`, `latitude`, `longitude`, `sog_kts` y `cog` se adaptan a las columnas estándar (`UTC`, `Lat`, `Lon`, `SOG`, `COG`) utilizadas por el visor.
- **Normalización temporal robusta:**  
  Los timestamps con zona horaria se convierten a **UTC real** y se gestionan como `datetime` naive, evitando conflictos en análisis y fusiones temporales.
- **Compatibilidad total con análisis existentes:**  
  El CSV de Vakaros se integra sin cambios en el flujo de métricas, detección de maniobras y análisis de tramos, incluyendo la columna **SOGS** requerida por el core.

#### maxSail GPX Cutter

- **Soporte ampliado de formatos de entrada:**  
  El GPX Cutter ahora acepta archivos **GPX, VKX y FIT** como entrada.
- **Normalización a GPX en memoria:**  
  Los archivos **VKX** y **FIT** se convierten internamente a formato GPX antes de aplicar el recorte, manteniendo intacta la lógica existente del cutter.
- **Separación clara de responsabilidades:**  
  maxSail-analytics continúa trabajando exclusivamente con **GPX y CSV**, delegando la normalización de formatos adicionales al GPX Cutter.
- **Base preparada para extensiones futuras:**  
  La arquitectura del cutter queda lista para preservar y transportar telemetría adicional (extensions GPX) en futuras versiones, sin afectar al flujo actual.

#### maxSail Metadata

- **Mejora tratamiento baliza:** - La autosincronización de la baliza ahora utiliza el **punto final del tramo filtrado** (último punto del track) en lugar del inicio.
  - Este cambio alinea la creación de balizas con la definición de marcas de recorrido a partir de tramos del track.
...
- **Improvement:** COG rose resolution is now configurable from the user interface.
  - Users can define the degrees per sector (COG_BIN_SIZE) to control the level of analysis detail.
- **Vakaros CSV support:**  
  maxSail-analytics now accepts CSV files generated by **Vakaros** devices, automatically normalizing their schema to the application's internal format.
- **Automatic column mapping:**  
  The `timestamp`, `latitude`, `longitude`, `sog_kts`, and `cog` fields are mapped to the standard columns (`UTC`, `Lat`, `Lon`, `SOG`, `COG`) used by the viewer.
- **Robust time normalization:**  
  Timestamps with time zone information are converted to **true UTC** and handled as naive `datetime` values, preventing issues in temporal analysis and merges.
- **Full compatibility with existing analysis:**  
  Vakaros CSV files integrate seamlessly into the existing metrics, maneuver detection, and leg analysis workflow, including the **SOGS** column required by the core.

#### maxSail GPX Cutter

- **Extended input format support:**  
  The GPX Cutter now accepts **GPX, VKX, and FIT** files as input.
- **In-memory GPX normalization:**  
  **VKX** and **FIT** files are converted to GPX in memory before applying the cutting logic, keeping the existing workflow unchanged.
- **Clear separation of responsibilities:**  
  maxSail-analytics continues to operate exclusively on **GPX and CSV**, while format normalization is handled by the GPX Cutter.
- **Future-ready architecture:**  
  The cutter architecture is prepared to preserve and transport additional telemetry (GPX extensions) in future releases, without impacting current behavior.

#### maxSail Metadata

- **Waypoint handling improved:** Waypoint autosync now uses the **end of the filtered segment**
  (last track point) instead of the start, aligning waypoint creation with race course
  mark definition from track segments.
