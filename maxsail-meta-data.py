...
utc_ini = t0 + pd.Timedelta(minutes=int(st.session_state.slider_rango[0]), seconds=int(st.session_state.seg_ini))
utc_fin = t0 + pd.Timedelta(minutes=int(st.session_state.slider_rango[1]), seconds=int(st.session_state.seg_fin))
if utc_fin < utc_ini:
    # Corrige inversión
...
with col_m1:
    twd = st.number_input("TWD (°)", 0, 359, int(st.session_state.meta.get("TWD", 0)), step=1, format="%d", key="twd")
    minuto_salida = st.number_input("Minuto salida", 0, 9999, int(st.session_state.meta.get("MINUTO_SALIDA", 5)), step=1, format="%d", key="minuto_salida")
with col_m2:
    tws = st.number_input("TWS (nudos)", 0, 60, int(st.session_state.meta.get("TWS", 0)), step=1, format="%d", key="tws")
    twsg = st.number_input("TWSG (rachas)", 0, 60, int(st.session_state.meta.get("TWSG", 0)), step=1, format="%d", key="twsg")

# El text_area nace con el valor que ya pusimos en session_state["notas"]
notas = st.sidebar.text_area("Notas personales", value=st.session_state.get("notas", ""), key="notas")

# Sincroniza meta con widgets actuales
...
def reset_baliza_temp() -> dict:
    ...
    if "df_filtro" in st.session_state and isinstance(st.session_state.df_filtro, pd.DataFrame) and not st.session_state.df_filtro.empty:
    ...
    return {"lat": lat, "lon": lon, "nombre": f"Baliza {len(st.session_state.balizas) + 1}"}

if "baliza_temp" not in st.session_state:
...
current_sig = (tuple(st.session_state.slider_rango), int(st.session_state.seg_ini), int(st.session_state.seg_fin))
...
with col_b1:
    _ = st.number_input("Latitud", format="%.5f", step=0.00005, key="lat_b_new", value=st.session_state.baliza_temp["lat"])
with col_b2:
    _ = st.number_input("Longitud", format="%.5f", step=0.00005, key="lon_b_new", value=st.session_state.baliza_temp["lon"])
_ = st.sidebar.text_input("Nombre", key="nombre_b_new", value=st.session_state.baliza_temp["nombre"])
...
balizas_edit_df = pd.DataFrame(st.session_state.balizas) if st.session_state.balizas else pd.DataFrame(columns=["lat", "lon", "nombre"])
...
def _lines_from_df(dfi: pd.DataFrame):
    ...
    seg = seg.dropna()
    return seg.rename(columns={"Lon": "from_lon", "Lat": "from_lat", "Lon_to": "to_lon", "Lat_to": "to_lat"}).to_dict("records")
...
def crear_layers(df, df_filtro, balizas_df, baliza_temp_df, show_temp):
    ...
    if len(df) > 1:
        layers.append(pdk.Layer(
            "LineLayer",
            data=[{"from":[r["from_lon"], r["from_lat"]], "to":[r["to_lon"], r["to_lat"]]} for r in _lines_from_df(df)],
    ...
    if len(df_filtro) > 1:
        layers.append(pdk.Layer(
            "LineLayer",
            data=[{"from":[r["from_lon"], r["from_lat"]], "to":[r["to_lon"], r["to_lat"]]} for r in _lines_from_df(df_filtro)],
            get_source_position="from",
            get_target_position="to",
            get_color="[0,0,0]",
            get_width=4,
    ...
    if show_temp and not baliza_temp_df.empty:
        layers.append(pdk.Layer(
            "ScatterplotLayer",
            data=baliza_temp_df.assign(name=baliza_temp_df.get("nombre", "Baliza temporal")),
            get_position='[lon, lat]',
...
balizas_confirmadas_df = pd.DataFrame(st.session_state.balizas) if st.session_state.balizas else pd.DataFrame(columns=["lat", "lon", "nombre"])
baliza_temp_df = pd.DataFrame([st.session_state.baliza_temp]) if st.session_state.show_temp else pd.DataFrame()
layers = crear_layers(df, df_filtro, balizas_confirmadas_df, baliza_temp_df, st.session_state.show_temp)
...
st.pydeck_chart(pdk.Deck(
    ...
    initial_view_state=pdk.ViewState(
        ...
        zoom=14,
        pitch=0,
        bearing=float(st.session_state.meta.get("TWD", 0)),
    ),
    layers=layers,
    tooltip=tooltip
))
...
with st.container():
    ...
    with col_time1:
        _ = st.slider(
            ...
            0, min_total, st.session_state.slider_rango,
            step=1,
            key="slider_rango"
        )
    with col_time2:
        _ = st.number_input("Segundos inicio", 0, 59, st.session_state.seg_ini, step=5, format="%d", key="seg_ini")
    with col_time3:
        _ = st.number_input("Segundos fin", 0, 59, st.session_state.seg_fin, step=5, format="%d", key="seg_fin")

# ---- Tabla resumen de balizas ----
print("DEBUG balizas columnas:", getattr(balizas_confirmadas_df, "columns", None))
...
if not balizas_confirmadas_df.empty:
    ...
    try:
        balizas_confirmadas_df["lat_str"] = balizas_confirmadas_df["lat"].map(lambda x: f"{x:.5f}")
        balizas_confirmadas_df["lon_str"] = balizas_confirmadas_df["lon"].map(lambda x: f"{x:.5f}")
